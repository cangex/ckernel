...
import unittest
from block_lifecycle_check import CASES, case_order, check_case, check_counters
...
class LifecycleTruth(unittest.TestCase):
    ...
    def inputs(self, case):
        s=CASES[case]; count=0 if s==2 else 1 if s==0 else 2; logs=[]; rows=[]
        for role in range(2):
            pid=101+role; parent=2000+role*10; size=4096 if s==0 else 8192
            lines=[f'CIS_SESSION_CONTAINER host_pid={pid}',
                f'CIS_LIFECYCLE_JOB role={role} scenario={s} begin_ns=10 end_ns=90 rc=0 original_bio={parent} bytes={size} requests={count} completed={int(s!=2)} io_errors={int(s==3)} canceled={int(s==2)} verified=1']
            for i in range(count):
                # Sequential request address reuse is valid; episode boundaries distinguish it.
                address=1000+role; bio=parent+i; begin=20+i*20; end=25+i*20; status=10 if s==3 else 0
