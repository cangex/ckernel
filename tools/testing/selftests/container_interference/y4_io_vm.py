...
from y3_filesystem_vm import filesystem_layout
...
def run():
    ...
    out=Path('/tmp/y4-io-evidence'); out.mkdir(mode=0o700)
    (out/'filesystem-layout.json').write_text(json.dumps(filesystem_layout(),indent=2))
    # Only the dedicated guest is configured; the host writeback policy is untouched.
    settings={'dirty_background_bytes':1048576,'dirty_bytes':4194304}
    settings_before={k:Path('/proc/sys/vm',k).read_text() for k in settings}
    for name,value in settings.items(): Path('/proc/sys/vm',name).write_text(str(value))
    root=Path('/sys/fs/cgroup/cis-y4-io'); root.mkdir(); (root/'management').mkdir()
    ...
    plan=dict(schema='cis-y4-io-plan-v1',order=case_order(),cases=CASES,window_ms=2000,
        bytes_per_actor=8388608,source=source,cpus=[0,1],manager_cpu=7,private_directories=True,
        settings_before=settings_before,guest_settings=settings,
        mounts=Path('/proc/self/mountinfo').read_text(),scope='public queues and native pauses, not unique blockers')
    (out/'plan.json').write_text(json.dumps(plan,indent=2))
    endpoint='/run/cis-y4-io.sock'; log=(out/'controller.log').open('x')
    ...
    def snapshot():
        return dict(before_ns=time.monotonic_ns(),proc_stat=Path('/proc/stat').read_text(),
            memory=Path('/proc/meminfo').read_text(),diskstats=Path('/proc/diskstats').read_text(),
    ...
    try:
        ...
        for entry in plan['order']:
            ...
            for fd in directories[0]:
                if libc.syncfs(fd): raise OSError(ctypes.get_errno(),'syncfs before window')
            before=snapshot()
            if entry['enabled']:
                sid=request('start',collector='block',targets=targets,nonce=label,window_ms=2000)['session_id']
            ...
            for i,p in enumerate(roots):
                ...
                child=subprocess.Popen(['/session_launch',str(p),str(i),'/io_pressure_workload',str(fd),str(i),str(start),label,c['mode']],
                    pass_fds=(fd,),stdout=h,stderr=h); children.append(child); running.append(child)
            codes=[p.wait(timeout=25) for p in running]; report=None; identities=None
            if sid:
            ...
            for i in range(2): os.unlink(label,dir_fd=directories[i][c['disks'][i]])
