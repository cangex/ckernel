...
insmod /cis_fixture.ko isolated_vm=1 && cp -a /dev/cis-fixture /container-root/cis-fixture && /usr/bin/python3 /profile/sync_vm.py
status=$?
echo "CIS_PROFILE_VM_EXIT=$status"
find /tmp/sync-evidence -type f | sort | while read -r f; do
    echo "CIS_FILE $f"
    cat "$f"
    echo
done
