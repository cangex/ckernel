...
for f in /tmp/session-evidence/controller.log /tmp/session-evidence/*-*.log /tmp/session-evidence/*-result.json /tmp/session-evidence/resource-inventories.json /tmp/session-evidence/fault-actions.jsonl /tmp/session-evidence/identity-requests.jsonl /tmp/session-evidence/records/* /tmp/session-evidence/identity-records/*; do
    [ -f "$f" ] || continue
    echo "CIS_FILE $f"
    cat "$f"
    echo
