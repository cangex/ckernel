...
if ! (insmod /virtio_blk.ko && insmod /cis_net_fixture.ko &&
      /bin/busybox ifconfig lo up && mkdir -p /container-root/dev &&
      cp -a /dev/cis-net-test /container-root/dev/); then
    echo CIS_PROFILE_VM_EXIT=91
    poweroff -f
    exit 91
fi
/usr/bin/python3 /profile/mixed_vm.py
status=$?
echo "CIS_PROFILE_VM_EXIT=$status"
find /tmp/mixed-evidence -type f | sort | while read -r f; do
    echo "CIS_FILE $f"
    cat "$f"
    echo
done
