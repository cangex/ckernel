...
def analyze(record,raw):
    ...
    pressure,pressure_defects=analyze_pressure(record,raw,requests,groups,tags)
    for pause in pressure['dirty_pauses']:
        pause['stack_leaf_to_root']=stacks.get(pause.pop('stack_id'),[])
    excluded.update(pressure_defects)
    ...
    if quality['status']!='PASS' or scope['status']!='PASS':
        ...
        pressure['tag_snapshots']=[]; pressure['wait_pool_associations']=[]; pressure['dirty_pauses']=[]
    else:
    ...
    return dict(schema='cis-block-report-v1',boot_id=record.get('boot_id'),session_id=record.get('session_id'),
        quality=quality,scope_audit=scope,requests=requests,tag_waits=tags,provenance=provenance,writeback=writeback,pressure=pressure,
        ...
        analysis_source_sha256=dict(base['analysis_source_sha256'],**{name:hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest()
            for name in ('block_report.py','block_tag_report.py','block_merge_report.py','writeback_report.py','io_pressure_report.py')}),
