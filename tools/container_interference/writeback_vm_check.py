...
def verify(serial,output):
    ...
    raw=serial.read_bytes(); text=raw.decode(); files=extract(text); prefix='/tmp/writeback-evidence/'
    ...
    if 'CIS_PROFILE_VM_EXIT=0' not in text.splitlines() or 'CIS_WRITEBACK_UNLOAD=0' not in text.splitlines():
    ...
    if (plan.get('order')!=case_order() or plan.get('rounds')!=3 or plan.get('bytes_per_actor')!=131072 or
            plan.get('buffered') is not True or plan.get('filesystem')!='ext4' or
            plan.get('sync_trigger')!='unregistered management syncfs' or plan.get('cpu')!=[0,1] or
            plan.get('window_ms')!=2000 or plan.get('management_cpu')!=7): errors.append('frozen_plan')
    ...
    for label in case_order():
        ...
        if '-block' in label:
            ...
            if (programs.get('required') is not True or programs.get('valid') is not True or
                    programs.get('programs')!=len(record['inventory']['program_names']) or programs.get('recursion_misses')!=0):
        ...
        result=check_case(label.split('-')[0],ev['window'],logs,ev['sync_interval_ns'],ev['verified'],report,identities)
