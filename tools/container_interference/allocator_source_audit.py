...
def parse(text):
    ...
    extended=h.get('version')=='2'
    if (set(h)!=({'version','active','shift','cache','snapshot','bytes_per_cpu'} | ({'release_active'} if extended else set())) or
            h['version'] not in ('1','2') or h['snapshot']!='non_atomic' or not h['cache'] or len(h['cache'])>=64):
    ...
    for line in lines[1:]:
        ...
        if set(row)!=(FIELDS|{'cpu'}|(FREE_FIELDS if extended else set())) or min(row.values())<0: raise ValueError('source row')
        cpu=row.pop('cpu')
...
def delta(before,after,shift=6,cache='maple_node'):
    ...
    fields=FIELDS | (FREE_FIELDS if ha['version']=='2' else set())
