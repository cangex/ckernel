...
def check(report, logs, case):
    if case not in ('threads','private','native'): raise ValueError('unsupported FD case')
    ...
    matched=0
    for edge in edges:
        if edge['relation']!='container_internal' or edge['process_scope']!='same_tgid': errors.append('false_cross_container_or_process')
    ...
    if case=='threads' and not matched: errors.append('missing_positive')
