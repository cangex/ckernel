...
def verify(serial, output):
    ...
    if plan.get('cases')!=['threads','private','native'] or plan.get('repetitions')!=3: errors.append('frozen_plan')
    expected={case+str(i) for i in range(3) for case in ('threads','private','native')}
    if {row.get('label') for row in declared['cases']}!=expected or len(declared['cases'])!=9: errors.append('declared_cases')
    ...
    if len(rows)!=9 or {row['nonce'] for row in rows}!=expected: errors.append('capture_set')
    for row in rows:
        ...
        truth=check(report,[files[prefix+label+'-%d.log'%i] for i in range(2)],label[:-1])
    ...
    result=dict(schema='cis-fd-vm-check-v1',status='FAIL' if errors else 'PASS',errors=sorted(set(errors)),cases=cases,
                ...
                pending=['explicit cross-container CLONE_FILES','FD destruction/address-reuse runtime','rwsem reader set'],
