...
def verify(serial,output):
    ...
    for label in order:
        ...
        result=(block_lifecycle_check.check_case(label.split('-')[0],ev['window'],logs,report,identities) if lifecycle else
                block_merge_check.check_case(label.split('-')[0],ev['window'],logs,report,identities) if merging else
        ...
        if lifecycle:
            result['fixture_counters']=block_lifecycle_check.check_counters(label.split('-')[0],ev['before']['fixture_counters'],ev['after']['fixture_counters'],result)
