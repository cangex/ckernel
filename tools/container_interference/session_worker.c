...
int main(int argc,char **argv)
{
	...
	for(i=roots_begin;i<argc;i++) {
		...
		if(sscanf(argv[i],"%c:%d:%llu:%llu%n",&role,&fd,&id,&gen,&n)!=4 || argv[i][n] ||
		   ...
		   (role=='i' && ctx->session_collector!=2 && ctx->session_collector!=6 && ctx->session_collector!=9 && ctx->session_collector!=10 && ctx->session_collector!=11 && ctx->session_collector!=12 && ctx->session_collector!=16 && ctx->session_collector!=17) ||
	...
	for(i=0;i<CIS_MAX_ROOTS;i++) if(ctx->roots[i].used && ctx->roots[i].session_target) {
		struct cis_root *r=&ctx->roots[i];
		if(ctx->session_collector>=2) {
			r->diagnostic_kind=(ctx->session_collector==2 || ctx->session_collector==6 || ctx->session_collector==12)?16:ctx->session_collector==3?1:ctx->session_collector==5?2:ctx->session_collector==7?32:(ctx->session_collector==8 || ctx->session_collector==13)?64:ctx->session_collector==9?128:ctx->session_collector==10?256:ctx->session_collector==11?512:ctx->session_collector==14?1024:ctx->session_collector==15?2048:ctx->session_collector==16?4096:ctx->session_collector==17?8192:4;
