...
int main(int argc,char **argv)
{
	...
	ctx->session_collector=!strcmp(argv[4],"ip")?1:!strcmp(argv[4],"owner")?2:
		!strcmp(argv[4],"sched")?3:!strcmp(argv[4],"reclaim")?4:!strcmp(argv[4],"sync")?5:!strcmp(argv[4],"fd")?6:!strcmp(argv[4],"counter")?7:!strcmp(argv[4],"allocator")?8:!strcmp(argv[4],"net")?9:!strcmp(argv[4],"block")?10:!strcmp(argv[4],"rwsem")?11:!strcmp(argv[4],"slub")?12:!strcmp(argv[4],"alloc_backend")?13:0;
	...
	for(i=0;i<CIS_MAX_ROOTS;i++) if(ctx->roots[i].used && ctx->roots[i].session_target) {
		struct cis_root *r=&ctx->roots[i];
		if(ctx->session_collector>=2) {
			r->diagnostic_kind=(ctx->session_collector==2 || ctx->session_collector==6 || ctx->session_collector==12)?16:ctx->session_collector==3?1:ctx->session_collector==5?2:ctx->session_collector==7?32:(ctx->session_collector==8 || ctx->session_collector==13)?64:ctx->session_collector==9?128:ctx->session_collector==10?256:ctx->session_collector==11?512:4;
