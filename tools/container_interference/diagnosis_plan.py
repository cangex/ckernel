...
def recommend(report, *, legacy=False):
    ...
    for row in report.get('candidates',[]):
        ...
        elif legacy and any(s.startswith(('alloc_fd','close_fd','put_files_struct','dup_fd','__fd_install')) for s in symbols):
        ...
        elif legacy and any(s.startswith(('__mutex_lock','mutex_lock','lockref_')) for s in symbols):
            selected=('owner','supported mutex/lockref candidate, object owner not yet known')
        elif legacy and any(s.startswith(('rwsem_','down_read','down_write','native_queued_spin','queued_spin','_raw_spin')) for s in symbols):
