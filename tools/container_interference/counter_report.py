...
def analyze(record, raw):
    ...
    for line in raw.decode().splitlines():
        ...
        if (not required <= d.keys() or d['protocol'] != 1 or d['operation'] not in OPERATIONS
    ...
    for key, rows in groups.items():
        ...
        valid = (quality_ok and first['stage'] == 1 and last['stage'] == 9 and
                 ...
                 all((r['leaf'], r['operation'], r['sample_shift'], r['stack_id']) ==
                     (first['leaf'], first['operation'], first['sample_shift'], first['stack_id']) for r in rows) and
        ...
        calls.append(dict(evidence='E1', actor=list(key[:4]), call_ns=key[-1], leaf_address=first['leaf'],
                          ...
                          object_lifetime='UNKNOWN', holder=None, spin_cycles=None))
    ...
    for call in calls:
        for step in call['steps']:
            ...
            participants[step['address'], field].add(tuple(call['actor']))
    candidates = [dict(address=address, field=field, actors=[list(a) for a in sorted(actors)],
                  ...
                  for (address, field), actors in sorted(participants.items()) if len({a[:2] for a in actors}) > 1]
    return dict(schema='cis-counter-report-v1', quality=base['quality'], scope_audit=audit(record, raw),
                ...
                coverage=dict(updates='IMPLEMENTED', rollback='IMPLEMENTED', lifetime='UNVERIFIED',
                ...
                limits=['operation wall time includes observer callbacks, preemption and nested work; not atomic-update cycles',
                        ...
                        'same address in different calls does not exclude object reuse; no inferred holder or blocker'])
