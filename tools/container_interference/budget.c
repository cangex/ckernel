...
void cis_budget_tick(struct cis_context *ctx,uint64_t now)
{
	...
	snprintf(detail,sizeof(detail),"user_cpu_ns=%llu rss_bytes=%llu errors=%u drops=%u maps_and_perf_budget_reserved_bytes=8388608",
	...
	if ((rss*(uint64_t)sysconf(_SC_PAGESIZE)+(8ULL<<20)>ctx->memory_limit) ||
