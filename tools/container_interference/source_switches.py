...
def expected_fields(version, collector):
    if version not in tuple(str(v) for v in range(1,21)):
        raise ValueError('unsupported source-switch version')
    v = int(version)
    if v < {'counter':2, 'allocator':3, 'alloc_backend':3, 'net':5, 'block':6, 'rwsem':7, 'slub':8, 'page_backend':16, 'filesystem':17, 'qdisc':19, 'cpu':20}.get(collector, 1):
        raise ValueError('unsupported source-switch version')
    ...
    if v >= 20:
        result.update(sched_switch=str(int(collector in ('cpu','owner','fd','slub'))),
                      sched_migrate=str(int(collector=='cpu')),sched_wait=str(int(collector in ('cpu','sched'))))
        result.update({k:str(int(collector=='cpu')) for k in ('irq_begin','irq_end','soft_begin','soft_end','work_begin','work_end')})
    return result
