...
def parse(text):
    ...
    if v in ('2','3','4'): expected.add('tx_active')
    if (set(header)!=expected or v not in ('1','2','3','4') or header['snapshot']!='non_atomic'):
    ...
    for line in lines[1:]:
        ...
        wanted=FIELDS|{'cpu'}|(TX_FIELDS if v in ('2','3','4') else set())
        wanted|=TX_CONTEXT_FIELDS if v=='3' else TX_PROCESS_FIELDS if v=='4' else set()
...
def delta(before,after,shift=0):
    ...
    totals={k:0 for k in FIELDS|(TX_FIELDS if a['version'] in ('2','3','4') else set())|
            (TX_CONTEXT_FIELDS if a['version']=='3' else TX_PROCESS_FIELDS if a['version']=='4' else set())}
    for cpu in ac:
        for key in totals:
    ...
    if a['version'] in ('2','3','4') and totals['tx_selected']>totals['tx_entries']: raise ValueError('TX source order')
    ...
    if a['version']=='4' and (totals['tx_nested_skipped']>totals['skipped'] or
