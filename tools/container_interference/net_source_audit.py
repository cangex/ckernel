...
TX_PROCESS_FIELDS={'tx_irq_filtered','tx_nested_skipped'}
RELEASE_FIELDS={'release_ends','release_callback_ns'}
...
def parse(text):
    ...
    if v in ('2','3','4','5'): expected.add('tx_active')
    if (set(header)!=expected or v not in ('1','2','3','4','5') or header['snapshot']!='non_atomic'):
    ...
    for line in lines[1:]:
        ...
        wanted=FIELDS|{'cpu'}|(TX_FIELDS if v in ('2','3','4','5') else set())
        wanted|=TX_CONTEXT_FIELDS if v=='3' else TX_PROCESS_FIELDS if v in ('4','5') else set()
        wanted|=RELEASE_FIELDS if v=='5' else set()
        if set(row)!=wanted or min(row.values())<0: raise ValueError('source row')
        cpu=row.pop('cpu')
...
def delta(before,after,shift=0):
    ...
    totals={k:0 for k in FIELDS|(TX_FIELDS if a['version'] in ('2','3','4','5') else set())|
            (TX_CONTEXT_FIELDS if a['version']=='3' else TX_PROCESS_FIELDS if a['version'] in ('4','5') else set())|
            (RELEASE_FIELDS if a['version']=='5' else set())}
    for cpu in ac:
        for key in totals:
    ...
    if a['version'] in ('2','3','4','5') and totals['tx_selected']>totals['tx_entries']: raise ValueError('TX source order')
    ...
    if a['version'] in ('4','5') and (totals['tx_nested_skipped']>totals['skipped'] or
            totals['tx_irq_filtered']+totals['tx_nested_skipped']+totals['tx_selected']>totals['tx_entries']):
        raise ValueError('TX process-context population audit')
    ...
    return dict(totals=totals,tx_callback_boot_max_ns=high_water,counter_bytes=a['source_counter_bytes_per_cpu']*len(ac),
                ...
                callback_ns_scope='TX and (v5) release callback bodies only; wrappers, filters and IRQ cost not fully separated',
